...
async fn get(path: &str, params: &str) -> Result<value::Value, error::Error> {
    ...
    let res = reqwest::get(&format!("{}{}/?{}", url, path, params))
        .await
        .unwrap()
        .text()
        .await
...
async fn compare() {
    ...
    let usage_system_kib = BufReader::new(f)
        .lines()
        .find_map(|line| {
            let line = line.ok()?;
    ...
    if diff.abs() < 10 * 1024 {
        println!(
            "system report: {}, memprof report: {}, difference: {}",
            usage_system_kib, usage_kib, diff
        );
    } else {
        panic!(
            "system report: {}, memprof report: {}, difference: {}",

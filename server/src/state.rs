...
impl Counters<u64> {
    pub fn diff(&self, other: &Self, elapsed_time: Duration) -> Counters<f64> {
        ...
        Counters {
            ...
            slab_unknown_alloc_count: d(
                self.slab_unknown_alloc_count,
                other.slab_unknown_alloc_count,
            ),
            slab_unknown_free_count: d(self.slab_unknown_free_count, other.slab_unknown_free_count),
            slab_unknown_bad_free_count: d(
                self.slab_unknown_bad_free_count,
                other.slab_unknown_bad_free_count,
            ),
            ...
            slab_known_bad_free_count: d(
                self.slab_known_bad_free_count,
                other.slab_known_bad_free_count,
            ),
...
impl AtomicState {
    ...
    pub fn process_event_all(&self, allocations: &mut HashMap<u64, u64>, event: &EventKind) {
        match event {
            &EventKind::KFree(ref v) => match allocations.get(&v.ptr.0) {
            ...
            &EventKind::CacheFree(ref v) => match allocations.get(&v.ptr.0) {

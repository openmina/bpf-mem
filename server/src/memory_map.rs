...
impl MemoryMapEntry {
    fn new(pid: u32) -> Result<Vec<Self>, io::Error> {
        ...
        File::open(&format!("/proc/{}/maps", pid))?.read_to_string(&mut entries)?;
...
impl FromStr for MemoryMapEntry {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let range = {
            ...
            let start = usize::from_str_radix(range_start, 16).map_err(|_| io::ErrorKind::Other)?;
            let end = usize::from_str_radix(range_end, 16).map_err(|_| io::ErrorKind::Other)?;
            start..end
        };
        ...
        let offset = usize::from_str_radix(offset_str, 16).map_err(|_| io::ErrorKind::Other)?;

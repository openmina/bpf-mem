...
mod event;
#[cfg(feature = "user")]
pub use self::event::{Event, EventKind, Stack};
...
#[cfg_attr(not(feature = "user"), allow(dead_code))]
...
impl Pod for KFree {
    ...
    fn from_slice(s: &[u8]) -> Option<Self> {
        ...
        Some(KFree {
            call_site: Hex64(u64::from_ne_bytes(
                TryFrom::try_from(&s[0x00..0x08]).unwrap(),
            )),
            ptr: Hex64(u64::from_ne_bytes(
                TryFrom::try_from(&s[0x08..0x10]).unwrap(),
            )),
        })
    }
}
...
impl Pod for KMAlloc {
    ...
    fn from_slice(s: &[u8]) -> Option<Self> {
        ...
        Some(KMAlloc {
            ...
            bytes_req: Hex64(u64::from_ne_bytes(
                TryFrom::try_from(&s[0x10..0x18]).unwrap(),
            )),
            bytes_alloc: Hex64(u64::from_ne_bytes(
                TryFrom::try_from(&s[0x18..0x20]).unwrap(),
            )),
            gfp_flags: Hex32(u32::from_ne_bytes(
                TryFrom::try_from(&s[0x20..0x24]).unwrap(),
            )),
        })
    }
}
...
impl Pod for KMAllocNode {
    ...
    fn from_slice(s: &[u8]) -> Option<Self> {
        ...
        Some(KMAllocNode {
            ...
            node: Hex32(u32::from_ne_bytes(
                TryFrom::try_from(&s[0x24..0x28]).unwrap(),
            )),
        })
    }
}
...
impl Pod for PageAlloc {
    ...
    fn from_slice(s: &[u8]) -> Option<Self> {
        ...
        Some(PageAlloc {
            pfn: Hex64(u64::from_ne_bytes(
                TryFrom::try_from(&s[0x00..0x08]).unwrap(),
            )),
            order: u32::from_ne_bytes(TryFrom::try_from(&s[0x08..0x0c]).unwrap()),
            gfp_flags: Hex32(u32::from_ne_bytes(
                TryFrom::try_from(&s[0x0c..0x10]).unwrap(),
            )),
...
impl Pod for PercpuAlloc {
    ...
    fn from_slice(s: &[u8]) -> Option<Self> {
        ...
        Some(PercpuAlloc {
            reserved: s[0x00] != 0,
            is_atomic: s[0x01] != 0,
            size: Hex64(u64::from_ne_bytes(
                TryFrom::try_from(&s[0x08..0x10]).unwrap(),
            )),
            align: Hex64(u64::from_ne_bytes(
                TryFrom::try_from(&s[0x10..0x18]).unwrap(),
            )),
            base_address: Hex64(u64::from_ne_bytes(
                TryFrom::try_from(&s[0x18..0x20]).unwrap(),
            )),
            off: i32::from_ne_bytes(TryFrom::try_from(&s[0x20..0x24]).unwrap()),
            ptr: Hex64(u64::from_ne_bytes(
                TryFrom::try_from(&s[0x28..0x30]).unwrap(),
            )),
        })
    }
}

    ...
    pub fn from_slice(slice: &[u8]) -> Result<Self, u8> {
        ...
        let (event, size) = match discriminant {
            ...
            x if Some(x) == PercpuAlloc::DISCRIMINANT => (
                EventKind::PercpuAlloc(PercpuAlloc::from_slice(slice).ok_or(0)?),
                PercpuAlloc::SIZE,
            ),
            x if Some(x) == PercpuFree::DISCRIMINANT => (
                EventKind::PercpuFree(PercpuFree::from_slice(slice).ok_or(0)?),
                PercpuFree::SIZE,
            ),
            x if Some(x) == AddToPageCache::DISCRIMINANT => (
                EventKind::AddToPageCache(AddToPageCache::from_slice(slice).ok_or(0)?),
                AddToPageCache::SIZE,
            ),
            x if Some(x) == RemoveFromPageCache::DISCRIMINANT => (
                EventKind::RemoveFromPageCache(RemoveFromPageCache::from_slice(slice).ok_or(0)?),
                RemoveFromPageCache::SIZE,
            ),
            _ => return Err(1),
        };
